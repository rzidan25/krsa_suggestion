...
krsa_filter_lowPeps_cycles <- function(raw_data, threshold, min_pct_cycles = 0.8,
                                      samples = NULL, groups = NULL) {
    raw_data %>%
        {
        ...
        dplyr::filter(ExposureTime == max(ExposureTime)) %>%
        dplyr::select(SampleName, Peptide, Cycle, Signal) %>%
        dplyr::group_by(Peptide, SampleName) %>%
        dplyr::summarise(pct_present = mean(Signal >= threshold, na.rm = TRUE), .groups = "drop") %>%
        dplyr::group_by(Peptide) %>%
        dplyr::filter(all(pct_present >= min_pct_cycles)) %>%
        dplyr::pull(Peptide) %>%
        unique() -> p

    message(paste(
        "Filtered out", length(unique(raw_data$Peptide)) - length(p),
        "Peptides not present in at least", paste0(round(min_pct_cycles * 100), "%"),
        "of cycles in every sample"
    ))
...
# ---- NEW: filter out peptides not consistently present across cycles ----
cycle_peptides <- krsa_filter_lowPeps_cycles(
  raw_data = qc_data,
  threshold = 5L,               # same as used later for low-signal filter
  min_pct_cycles = 0.8          # keep peptides present in ≥80% of cycles
)

# Keep only those peptides for all downstream steps
qc_data_filtered <- qc_data |>
  dplyr::filter(Peptide %in% cycle_peptides)

# extract end level signal values @ all max exposure time (100ms)
data_pw_max <- krsa_extractEndPointMaxExp(qc_data_filtered, chipType)

# extract end level signal values end level @ all exposure times
data_pw <- krsa_extractEndPoint(qc_data_filtered, chipType)
